...
adjust_dates() {
    ...
    # MySQL general log and slow log: YYYY-MM-DD HH:MM:SS
    patterns["mysql_general"]='[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\} [0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\}'
    # MySQL slow log with '# Time: '
        ...
        for orig_date in $original_dates; do
            ...
            case $pattern_name in
                nginx_access)
                    ...
                    old_datetime=$(echo "$orig_date" | tr -d '[]')
                ...
                nginx_error)
                    old_date="$orig_date"
                    new_date_formatted=$(date -d "$new_date" '+%Y/%m/%d %H:%M:%S')
                    old_date_esc=$(echo "$old_date" | sed 's/[\/&]/\\&/g')
                    new_date_esc=$(echo "$new_date_formatted" | sed 's/[\/&]/\\&/g')
                    file_content=$(echo "$file_content" | sed "s/$old_date_esc/$new_date_esc/g")
                    ;;
                mysql_general)
